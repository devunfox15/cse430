const dynamicMessage = document.querySelector('.dynamicMessage') as HTMLElement | null;
const userNameInput = document.getElementById('userNameInput') as HTMLInputElement | null;
const updateNameButton = document.getElementById('updateNameButton') as HTMLButtonElement | null;
const keyWordsElement = document.querySelector('.KeyWords') as HTMLElement | null;
const errorElement = document.querySelector('.error') as HTMLElement | null; // Target the error div

// Add event listener to update message on button click
updateNameButton?.addEventListener('click', () => {
    if (userNameInput?.value) {
        const enteredName: string = userNameInput.value.trim();

        // Check if the entered name contains numbers
        if (/\d/.test(enteredName)) {  // \d checks for any digits (0-9)
            // Add a new <p> tag to the .error div with the error message
            if (errorElement) {
                const errorParagraph = document.createElement('p'); // Create a new <p> element
                errorParagraph.className = "text-2xl text-center pt-6"; // Add the classes
                errorParagraph.textContent = "Error: Numbers are not allowed for this invite!";
        ...
        if (enteredName === "Devun Fox Durst") {
            ...
            if (dynamicMessage) {
                dynamicMessage.innerHTML = `My name is <span class="text-[#391585]">Devun Fox Durst</span>`;
            }
        ...
        if (dynamicMessage) {
            dynamicMessage.textContent = `Hey, ${enteredName}, You seem cool. Connect with me on LinkedIn!`;
            if (keyWordsElement) {
